...
class OBSDSubmesh {
    ...
    init(submesh: MTKSubmesh, mdlSubmesh: MDLSubmesh, vertexFunctionName: String, fragmentFunctionName: String) {
        ...
        pipelineState = OBSDSubmesh.makePipelineState(textures: textures, vertexFunctionName: vertexFunctionName, fragmentFunctionName: fragmentFunctionName)
    }
}

private extension OBSDSubmesh {
...
extension OBSDSubmesh: Texturable {}

private extension OBSDSubmesh.Textures {
    init(material: MDLMaterial?) {
        func property(with semantic: MDLMaterialSemantic) -> MTLTexture? {
            guard let property = material?.property(with: semantic),
                property.type == .string,
                let filename = property.stringValue,
                let texture = ((try? OBSDSubmesh.loadTexture(imageName: filename, bundle: Bundle.main)) as MTLTexture??) else {
                    return nil
            }
            return texture
        }
